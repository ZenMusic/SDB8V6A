...
namespace SymbolDB
{
    public partial class DisplayPreviewSet : Form
    {
        ...
        public DisplayPreviewSet(GlobalVars g, /*ImageCache cache, */ int startingImageNumber = 0, int numberOfRows = 6) /////////////////////// main init
        {
            InitializeComponent();
        ...
        // Helper to safely replace PictureBox.Image on UI thread while disposing previous Image.
        private void SetPictureBoxImage(int idx, Image newImage)
        {
            if (pb == null) return;
            if (idx < 0 || idx >= pb.Length) return;
            var control = pb[idx];
        ...
        public int LoadSetAI(int startingImageNo)
        {
            ...
            if (bUSE_OPTIMIZED_LOAD && startingImageNo == prevStart + 1)
            {
                if (count >= 2)
                {
                    // shift left safely using actual loaded count
                    for (int i = 0; i < count - 1; ++i)
                        MoveImageOwnership(i + 1, i);

                    // update tooltips to reflect new mapping
                    for (int i = 0; i < count - 1; ++i)
                    {
                        int fileIdx = startingImageFileIdx + i;
                        if (fileIdx < gv.imageFileList1.getImageCount())
                            pbToolTip.SetToolTip(pb[i], gv.imageFileList1.getIndexed(fileIdx).fpath);
                    }

                    SetPictureBoxImage(count - 1, null);
                    iStartPb = count - 1;
                    singleStep = true;

                    int loadIndex = startingImageFileIdx + (count - 1);
                    if (loadIndex < gv.imageFileList1.getImageCount())
                        Load1Image(loadIndex, count - 1);
                }
            }
            else if (bUSE_OPTIMIZED_LOAD && startingImageNo == prevStart - 1)
            {
                if (count >= 2)
                {
                    // shift right safely using actual loaded count
                    ...
                    SetPictureBoxImage(0, null);
                    iStartPb = 0;
                    singleStep = true;

                    if (startingImageFileIdx < gv.imageFileList1.getImageCount())
                        Load1Image(startingImageFileIdx, 0);
                }
            }

            if (!singleStep)
                rc = LoadFromImageList2(startingImageNo, iStartPb);

            for (int i = 0; i < LoadedCount(); i++)
            {
                int idx = startingImageFileIdx + i;
                if (idx < gv.imageFileList1.getImageCount())
                    LoadThumbnailAsync(i, gv.imageFileList1.getIndexed(idx).fpath);
            }

            setTitle($"{startingImageFileIdx} thru {rc} of {gv.slideCount1}  » {pb[0].ImageLocation}");
            tbLoading.Hide();
            btLoadingNextSet.SendToBack();
            ResumeLayout();
            return rc;
        }


        // Add this method inside the DisplayPreviewSet class, near other private helpers

        /// <summary>
        /// Moves the Image reference from one PictureBox to another, disposing the old image in the destination.
        /// </summary>
        private void MoveImageOwnership(int fromIdx, int toIdx)
        {
            if (pb == null) return;
            if (fromIdx < 0 || fromIdx >= pb.Length) return;
            if (toIdx < 0 || toIdx >= pb.Length) return;
            var src = pb[fromIdx];
            var dst = pb[toIdx];
        ...
        public int LoadSet(int startingImageNo) //// load images ///////////////////////////////////////////////////////////////////
        {
            ...
            if (startingImageNo < 0)
                ...
                startingImageNo = Math.Max(0, gv.imageFileList1.getImageCount() - LoadedCount());
            startingImageFileIdx = startingImageNo;
            ...
            if (!skipMemFree)
            {
                for (int idx2 = 0; idx2 < LoadedCount(); ++idx2)
                    SetPictureBoxImage(idx2, null);
            }
            ...
            if (bUSE_OPTIMIZED_LOAD)
            {
                if (startingImageNo == previousStartingFileIdx + 1) // step forward
                {
                    if (count >= 2)
                    {
                        for (int idx = 0; idx < count - 1; ++idx)
                            MoveImageOwnership(idx + 1, idx);

                        // reset tooltips to new mapping
                        for (int idx = 0; idx < count - 1; ++idx)
                        {
                            int fileIdx = startingImageFileIdx + idx;
                            if (fileIdx < gv.imageFileList1.getImageCount())
                                pbToolTip.SetToolTip(pb[idx], gv.imageFileList1.getIndexed(fileIdx).fpath);
                        }
                ...
                else if (startingImageNo == previousStartingFileIdx - 1) // step back
                {
                    if (count >= 2)
                    {
                                ...
                                else // path okay
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            try
                                            {
                                                // Dispose old before pb.Load if any
                                                var old = pb[nextpb].Image;
        ...
        public bool LoadPbFromThumbnail(int pbx, string fpathname)
        {
            ...
            SetPictureBoxImage(pbx, thumbnail);
        ...
        public bool LoadPbFromStream(int pbx, string fpathname)
        {
            ...
            try
            {
                // create image from stream, then replace image on pb disposing old
                var img = System.Drawing.Image.FromStream(fileStream2);
                SetPictureBoxImage(pbx, img);
            }
                        ...
                        if (idxImageFileItem < gv.slideCount1)
                        {
                            if (nextpb >= iStartPbNum)
                            {
                                ...
                                else
                                {
                                    ...
                                    try
                                    {
                                        // dispose old before Load
                                        SetPictureBoxImage(nextpb, null);
                                        pb[nextpb].Load(fpath);//openFileDialog1.FileName); ///////////////////// nextpb 
                                    }
                                    catch (Exception)
                                    {
        ...
        private void MarkPbImageForDeletion(int pbIndex)
        {
            if (pbIndex < 0) return;
            int imageIdx = pbIndex + startingImageFileIdx;
            try
            {
                ...
                if (pb[pbIndex] != null)
                {
                    pb[pbIndex].BackColor = Color.LightCoral;
                }
            }
            catch
            {
        ...
        private void PbClicked(object sender, MouseEventArgs e)
        {
            ...
            // Ctrl + Left-click -> mark for deletion
            if (gv.bAllowDeleteWithControlClick)
            {
                if (e.Button == MouseButtons.Left && (Control.ModifierKeys & Keys.Control) == Keys.Control)
                {
                    MarkPbImageForDeletion(idx);
                    return;
                }
            }
            else if ((Control.ModifierKeys & Keys.Control) == Keys.Control)
            {
                ...
                MoveToTopAndDisplay1();
                this.Close();
                return;
            }

            // Shift handling (unchanged)
            ...
            {
                // Right-click: show context menu (unchanged)
                DisplaySet_MouseDown(sender, e);
            ...
            {
                // other left-click behavior remains (selection / copy / rename)
                if (false)
                {
                    ...
                    lastSelectedPbIdx = idx;
                    pb[idx].Select();
                    gv.debug.w("selected PB", idx.ToString(), " imagesDisplyed ", maxPbIdxLoaded.ToString());
                    pb[idx].BorderStyle = BorderStyle.Fixed3D;
                    if (bcopy)
                        copyImage(pb[idx].ImageLocation, gv.targetFolder);
                    else if (brename)
                        renameImage();
                }
            }
        ...
        private void PbClicked222(object sender, MouseEventArgs e)
        {
            int x = e.X;
            int y = e.Y;
            int idx = 0;
...
*/
        ...
        private void copyImage(string fpath, string targetDir)
        {
            ...
            ff.CopyFileRename(fpath, gv.targetFolder, newName);
            renameSeq++;
        }
    ...
    * */
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData) /////// override dmcdmc123
        {
            ...
            {
                // clamp left move so we never ask for a start < 0
                hideDisplayFileName();
                stopTimer();
                int desiredLeft = this.startingImageFileIdx - 1;
                int safeLeft = ClampStartIndex(desiredLeft);
                if (safeLeft != this.startingImageFileIdx)
                    this.LoadSet(safeLeft);
                return true;
            }
            else if (keyData == Keys.Right) //right arrow 
            {
                // clamp right move so we never go past last page
                hideDisplayFileName();
                int desiredRight = this.startingImageFileIdx + 1;
                int safeRight = ClampStartIndex(desiredRight);
                if (safeRight != this.startingImageFileIdx)
                    this.LoadSet(safeRight);
                return true; //for the active control to see the keypress, return false
            }
            else if (keyData == Keys.Down) // down arrow 
            {
                hideDisplayFileName();
                int desiredDown = this.startingImageFileIdx + numColumns;
                int safeDown = ClampStartIndex(desiredDown);
                this.LoadSet(safeDown);
                return true; //for the active control to see the keypress, return false
            }
            else if (keyData == Keys.PageDown)
            {
                ...
                int desired = this.startingImageFileIdx + maxImagesToLoad;
                int safeStart = ClampStartIndex(desired);

                this.LoadSet(safeStart); // safely clamped
            ...
            {
                // Use the actual number of loaded slots (LoadedCount) so we don't overshoot
                hideDisplayFileName();
                int desired = gv.slideCount1 - LoadedCount();
                if (desired < 0) desired = 0;
                int safeStart = ClampStartIndex(desired);
                this.LoadSet(safeStart);  // go to last page of images, safely clamped
                return true;
            }
            else if (keyData == Keys.Home)
            {
                hideDisplayFileName();
                this.LoadSet(0);  //go to first page of images
                return true;
            }
        ...
        private void renameSeqToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                gv.debug.w("RENAMEcopy 0 > ON < to target ", gv.targetFolder);
                bcopy = true;
        ...
        private void display1ToolStripMenuItem_Click(object sender, EventArgs e) // DISPLAY SELECTED PB IMAGE in 1 and main display
        {
            if (lastSelectedPbIdx >= 0)
            {
                gv.debug.displayThisImage(pb[lastSelectedPbIdx].Image);
        ...
        private void toolStripMenuItem1_Click(object sender, EventArgs e) //MOVE TO TOP 
        {
            if (lastSelectedPbIdx >= 0)
            {
                int imageIdx = lastSelectedPbIdx + startingImageFileIdx;
        ...
        // Add these helpers into the DisplayPreviewSet class (near other private helpers like ClampStartIndex and MoveImageOwnership)

        private int LoadedCount()
        {
            // number of PictureBox slots currently considered loaded/displayed
            // maxPbIdxLoaded is maintained as "last loaded index" (or -1 when none)
            return Math.Max(0, maxPbIdxLoaded + 1);
        }
        /// <summary>
        /// Clamps the requested start index to a valid range for image paging.
        /// </summary>
        private int ClampStartIndex(int requestedStart)
        {
            int totalImages = gv.imageFileList1.getImageCount();
            int maxStart = Math.Max(0, totalImages - maxImagesToLoad);
            if (requestedStart < 0)
                return 0;
        ...
        private void ShiftRightSafely()
        {
            int idx;
            int count = LoadedCount();
            if (count < 2) return;

            // backward (step left) optimized shift — replace the existing loop that calls SetPictureBoxImage(...)
            for (idx = maxPbIdxLoaded; idx > 0; --idx) //pbCreatedCount
            {
                // Transfer ownership from pb[idx-1] -> pb[idx] safely (disposes old dst image and nulls src)
                MoveImageOwnership(idx - 1, idx);
            }
