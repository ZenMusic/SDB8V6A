            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                Parallel.ForEach(allFiles, po, file =>
                {
                    ...
                    {
                        // if (count % updateAfterCount == 0) ProgressCallback?.Invoke(count, matched);
                        // decide whether to report every 100 or every 1,000
                        reportInterval = (count <= 1_000) ? 100 : 1_000;
                    ...
                    try
                    {
                        using var meta = new Bitmap(file);
                        if (meta.Width != mainImage.Width || meta.Height != mainImage.Height)
                        {
                            //if (count % updateAfterCount == 0) ProgressCallback?.Invoke(count, matched);
                            // decide whether to report every 100 or every 1,000
                            reportInterval = (count <= 1_000) ? 100 : 1_000;
                    ...
                    try
                    {
                        using var testOrig = new Bitmap(file);
